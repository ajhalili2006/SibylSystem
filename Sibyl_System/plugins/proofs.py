...
async def make_proof(event, proof_id):
    ...
    return proof_string.format(
        proof_id=proof_id, reason=reason, paste=paste, url=url)
    ...
    if query.startswith("qproof"):
        if len(split) == 1:
            ...
            if not user_data:
                result = builder.article(
                    'User is not gbanned',
                    text=f'User[{split[1]}] is not gbanned')
            else:
        ...
        if len(split) == 1:
            result = builder.article(
                "Type Case-ID", text="No Case-ID was provided")
        else:
            proof = await make_proof(event, int(split[1]))
            if proof == "Invalid":
                result = builder.article(
                    "Invalid  Case-ID", text="Case-ID is Invalid")
            elif proof == "Media":
                result = builder.article(
                    "The provided message was media",
                ...
                result = builder.article(
                    "Proof", text=proof, link_preview=False)
    await event.answer([result])
