...
async def scan(event):
    ...
    if re.match('.scan (-f )?-o .*', event.text) or re.match(
            ".scan -o .*", event.text) or re.match(".scan (-f )?-p",
                                                   event.text):
        if replied.fwd_from:
...
async def revive(event):
    ...
    await a.edit(
        "Revert request sent to sibyl. This might take 10minutes or so.")


@System.on(
    system_cmd(pattern=r'approve', allow_inspectors=True, force_reply=True))
    ...
    if match:
        ...
        if reply == me.id:
            ...
            try:
                ...
                bot = False
            await System.gban(
                enforcer, scam, reason, replied.id, sender, bot=bot)
...
@System.on(
    system_cmd(pattern=r'reject', allow_inspectors=True, force_reply=True))
async def reject(event):
    ...
    if orig:
        await System.send_message(
            orig.group(1), 'Scan rejected.', reply_to=int(orig.group(2)))
