...
class SibylClient(TelegramClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if BOT_TOKEN:
            self.bot = TelegramClient(
                "SibylSystem", api_id=API_ID_KEY,
                api_hash=API_HASH_KEY).start(bot_token=BOT_TOKEN)
    ...
    async def gban(self,
        ...
        if bot:
            await self.send_message(
                Sibyl_approved_logs,
                bot_gban_string.format(
                    enforcer=enforcer, scam=target, reason=reason))
        else:
            await self.send_message(
                Sibyl_approved_logs,
